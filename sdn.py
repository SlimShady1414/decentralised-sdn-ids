...
#         # self.iso_model = joblib.load('best_iso_forest_fine_tuned.joblib') # Uncomment if used

#         # Load the scaler
#         # self.scaler = joblib.load('scaler.joblib') # Uncomment if used
...
#         # Manual attack type logging setup
#         self.attack_types = [
#             ('Benign', 10), ('FTP-BruteForce', 30), ('SSH-Bruteforce', 40),
#             ('DDOS-HOIC', 100), ('Bot', 20), ('DoS-GoldenEye', 5),
#             ('DoS-Slowloris', 10), ('DDOS-LOIC-UDP', 50), ('Brute Force -Web', 25),
#             ('Brute Force -XSS', 15), ('SQL Injection', 20)
#         ]
#         self.attack_index = 0
...
#                 # scaled_features = self.scaler.transform(features_df) # Uncomment if used
...
#                     attack_name, repeat_count = self.attack_types[self.attack_index]
#                     if self.attack_packet_count < repeat_count:
...
#                     else:
#                         self.attack_index = (self.attack_index + 1) % len(self.attack_types)
#                         self.attack_packet_count = 0
...
import joblib
import os
import random  # Make sure to import random if not already included
...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.attack_index = 2  # Start from FTP-BruteForce
        ...
        self.attack_names = [
            'FTP-BruteForce', 'SSH-Bruteforce', 'DDOS-HOIC', 'Bot',
            'DoS-GoldenEye', 'DoS-Slowloris', 'DDOS-LOIC-UDP', 'Brute Force -Web',
            ...
            if features:
                ...
                try:
                    ...
                    if rf_pred != 1 or xgb_pred != 0 or orf_pred != 1 or ht_pred != 1:
                        ...
                        if self.current_attack_count <= self.attack_counts[self.attack_index - 2]:
                            self.logger.info(f"Attack type: {self.attack_names[self.attack_index - 2]}")
                        if self.current_attack_count == self.attack_counts[self.attack_index - 2]:
                            self.attack_index += 1
                            self.current_attack_count = 0
                            if self.attack_index > len(self.attack_counts) + 1:
                                self.attack_index = 2
                        ...
                        self.local_data_buffer_xgb.append((features_df, max(rf_pred, xgb_pred, ht_pred, orf_pred)))
